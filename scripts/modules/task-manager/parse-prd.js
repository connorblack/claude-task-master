/**
 * PRD Parser with OpenAI Structured Output Compatibility
 *
 * IMPORTANT: This file contains a workaround for OpenAI's structured output limitations
 * when using generateObject with Zod schemas. The issue is documented here:
 * https://github.com/vercel/ai/issues/3201
 *
 * Problem: OpenAI's structured output parser has issues with:
 * 1. Schema references (using one Zod schema within another)
 * 2. .optional().default() modifiers not being included in required arrays
 * 3. Dynamic/derived schemas passed to generateObject
 *
 * Solution Applied:
 * - Inlined the task schema instead of referencing prdSingleTaskSchema
 * - Removed .optional().default() modifiers from schema
 * - Added manual default value handling in task processing
 * - All properties must be listed in required arrays for OpenAI compatibility
 * - Added .strict() to prevent additional properties
 *
 * When the upstream issue is resolved, we can revert to the cleaner schema approach.
 */
...
// ORIGINAL SCHEMA - Commented out due to OpenAI structured output compatibility issues
// See: https://github.com/vercel/ai/issues/3201
// The issue: OpenAI's structured output parser doesn't handle schema references well
// when using generateObject with dynamic/derived schemas
//
...
// WORKAROUND: Inlined schema to avoid OpenAI structured output parser issues
// - Removed schema references (prdSingleTaskSchema) and inlined the task schema
// - Removed .optional().default() modifiers that weren't being included in required arrays
// - All properties must be listed in required arrays for OpenAI's structured output
// - Added .strict() to prevent additional properties
//
// TODO: Revert to cleaner schema approach when https://github.com/vercel/ai/issues/3201 is resolved
const prdResponseSchema = z
  .object({
    tasks: z.array(
      z
        .object({
		...
		Return your response in this format:
{
    ...
    const processedNewTasks = generatedData.tasks.map((task) => {
      ...
      return {
        ...
        // Apply default values that were removed from schema due to OpenAI compatibility
    ...
    const finalTasks =
      append ? [...existingTasks, ...processedNewTasks] : processedNewTasks
